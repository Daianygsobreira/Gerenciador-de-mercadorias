...
function EstoqueGrafico({ mercadoria, onCancel }) {
    ...
    useEffect(() => {
        let url = mercadoria ? `/mercadorias/${mercadoria.id}/resumo_mensal` : '/movimentacoes_estoque/resumo_mensal';
        api.get(url)
        .then(resposta => {
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead>
                            <tr style={{ backgroundColor: '#f2f2f2', textAlign: 'left' }}>
                                <th style={{ border: '1px solid #dddddd', padding: '8px' }}>{ mercadoria ? 'Mês' : 'Produto'}</th>
